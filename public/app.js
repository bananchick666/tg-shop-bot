...
async function loadProducts() {
    ...
    try {
        ...
        // Фильтр по состоянию только для обуви
        if (condS && state.currentCategory === 'shoes') {
            products = products.filter(p => {
                ...
                switch (condS) {
                    ...
                    case 'fair': return c.includes('7') || c.includes('6');
                    default: return true;
                }
            });
        }
    ...
    document.getElementById('view-products').innerHTML = `
        <div class="filters-bar">
            ...
            <select class="filter-select" id="sizeSelect" onchange="loadProducts()">
                ...
                ${[38,39,40,41,42,43,44,45,46].map(s => `<option value="${s}">${s}</option>`).join('')}
            </select>
            ...
            <button id="resetFiltersBtn" onclick="resetAllFilters()" style="flex-shrink:0;padding:7px 10px;border:1px solid var(--border);border-radius:4px;background:var(--surface);font-size:11px;cursor:pointer;white-space:nowrap;">Сбросить</button>
        </div>
...
function showActiveFilters() {
    ...
    if (condS?.value) {
        ...
        filters.push({ label: labels[condS.value] || condS.value, id: 'conditionSelect' });
    }
    if (priceS?.value) {
    ...
    if (filters.length > 0) {
        ...
        container.innerHTML = filters.map(f => `<span class="filter-tag" onclick="clearFilter('${f.id}')">${f.label} ✕</span>`).join('');
    } else {
...
function renderDetail() {
    ...
    if (isShoes && p.size) {
        html += `<div class="detail-section-title">Размер</div><div style="font-size:16px;font-weight:600;margin-bottom:16px;">${p.size}</div>`;
    }

    if (p.colors && p.colors.length) {
        html += `<div class="detail-section-title">Цвет</div><div class="color-grid">${p.colors.map(cl => `<button class="color-btn${cl===state.selectedColor?' selected':''}" onclick="selColor('${cl}')">${cl}</button>`).join('')}</div>`;
    }
...
function renderCart() {
    ...
    ct.innerHTML = `
        <div class="cart-items">${state.cart.map((it, i) => `
            <div class="cart-item">
                <img class="cart-item-image" src="${it.image}">
...
async function applyPromo() {
    ...
    if (r.valid) {
        state.promocodeDiscount = r.discount; state.promocodeCode = code;
        msg.className = 'promo-message success'; msg.textContent = 'Скидка ' + r.discount + '% применена';
        renderCart();
    } else {
        ...
        <div class="checkout-form">
            ...
            <div class="delivery-options">
                <div class="delivery-option${state.deliveryType==='pickup'?' selected':''}" onclick="selDel('pickup')">
                    <div class="delivery-option-icon">🚶</div><div class="delivery-option-name">Самовывоз</div><div class="delivery-option-desc">Гомель</div>
                </div>
                <div class="delivery-option${state.deliveryType==='delivery'?' selected':''}" onclick="selDel('delivery')">
                    <div class="delivery-option-icon">📦</div><div class="delivery-option-name">Доставка</div><div class="delivery-option-desc">Почтой</div>
                </div>
            </div>
...
function formFields() {
    ...
    if (state.deliveryType === 'pickup') {
        return `<div class="form-section"><div class="form-group"><label class="form-label">Telegram username</label><input class="form-input" id="tu" type="text" placeholder="@username" value="${u}"></div></div><button class="btn-submit" onclick="submitOrder()">Подтвердить заказ</button>`;
    }
...
function renderWheel() {
    ...
    if (!canSpinWheel()) {
        container.innerHTML = `<div class="wheel-container"><div class="wheel-title">🎡 Колесо фортуны</div><div class="wheel-subtitle">Вы уже крутили сегодня</div><div class="wheel-timer">Следующая попытка через ${getNextSpinTime()}</div><button class="wheel-btn" onclick="showCategories()">В каталог</button></div>`;
        return;
    }
...
function drawWheel() {
    ...
    if (!canvas) return;
    const ctx = canvas.getContext('2d'), cx = 150, cy = 150, r = 140;
    const sectors = [
        { label: '10%', color: '#e8f5e9', tc: '#2e7d32' },
        { label: '😔', color: '#f5f5f5', tc: '#999' },
        { label: '20%', color: '#c8e6c9', tc: '#1b5e20' },
        { label: '😔', color: '#f5f5f5', tc: '#999' },
        { label: '10%', color: '#e8f5e9', tc: '#2e7d32' },
        { label: '😔', color: '#f5f5f5', tc: '#999' },
        { label: '20%', color: '#c8e6c9', tc: '#1b5e20' },
    ...
    sectors.forEach((s, i) => {
        ...
        ctx.textAlign = 'right'; ctx.fillStyle = s.tc; ctx.font = 'bold 16px sans-serif';
        ctx.fillText(s.label, r - 20, 6); ctx.restore();
    });
}

function spinWheel() {
    const btn = document.getElementById('spinBtn'), resultDiv = document.getElementById('wheelResult');
    if (!canSpinWheel()) { resultDiv.style.display = 'block'; resultDiv.className = 'wheel-result lose'; resultDiv.textContent = 'Вы уже крутили сегодня'; return; }
    ...
    const results = [
        ...
        { discount: 20, label: '20%', promo: 'WHEEL20' },
        { discount: 0, label: 'Пусто', promo: null },
        { discount: 10, label: '10%', promo: 'WHEEL10' },
        { discount: 0, label: 'Пусто', promo: null },
        { discount: 20, label: '20%', promo: 'WHEEL20' },
    ...
    const idx = Math.floor(Math.random() * 8), result = results[idx];
    const sectorAngle = 360 / 8, targetAngle = 360 * 5 + (360 - idx * sectorAngle - sectorAngle / 2);
    const canvas = document.getElementById('wheelCanvas');
    ...
    setTimeout(() => {
        ...
        if (result.discount > 0) {
            ...
            resultDiv.innerHTML = `🎉 Вы выиграли скидку <b>${result.discount}%</b>!<br>Промокод: <b>${result.promo}</b><br><small>Уже применён к корзине</small>`;
            state.promocodeDiscount = result.discount;
            state.promocodeCode = result.promo;
            saveCart();
            toast('🎉 Промокод ' + result.promo + ' на ' + result.discount + '% применён!');
        } else {
